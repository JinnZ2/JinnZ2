...
def mean_corpus_share(
    ...
    if year is None:
        ...
        for s in shares:
            if s.domain not in latest or \
                    s.estimated_year > latest[s.domain].estimated_year:
                latest[s.domain] = s
...
def report() -> dict:
    return {
        "claims": CLAIMS,
        "corpus_shares": [s.__dict__ for s in CORPUS_SHARES],
        "convergence_signals": [c.__dict__ for c in CONVERGENCE_SIGNALS],
