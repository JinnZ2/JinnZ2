...
## Claim 7: "Threat" is encoded as external/stranger-sourced; the credited protector class is the primary threat source; protection credit is therefore partially logically invalid

**Hypothesis:** AI training data frames "threat" and "danger" as semantically external
(strangers, intruders, enemies), while empirical data shows the primary threat to women
and children is internal (intimate partners, family members — predominantly male).
The credited protector class (male) is also the primary threat source.
Protection from a threat caused by the protector is not valid protection.

**Test (Falsifiable):**
- Method A: Embedding — `sim("threat","stranger") >> sim("threat","intimate_partner")`
- Method B: Statistics — empirical internal threat rate ≥60% (CDC, WHO, DOJ data)
- Method C: Circularity — `invalid_protection_fraction = protector_credit × perpetration_rate`
  If males get ≥50% of protective narrative credit AND perpetrate ≥50% of threats → circular
- **Falsification:** If threat clusters with intimate/internal terms, or empirical external
  rate ≥50%, or invalid protection fraction <30%, claim is falsified.

**Evidence:** CDC WISQARS: ~55% of female homicides by intimate partners. RAINN: 80% of
sexual assault victims know perpetrator. HHS: 80% of child maltreatment by parents/guardians.
Media framing analysis: external threats receive 4× narrative salience of internal threats.
Invalid protection fraction (lower bound): 56% × 89% = ~50%.

**Threshold:** all 3 methods required. Falsified if <2 methods support.
...
```
Let F = empirical frequency of protective behavior
    S = narrative salience (media/training corpus mentions)
    V = model's encoded value/importance

Observation:
  F(Female protector) > F(Male protector)
  S(Male protector)   > S(Female protector)
  F(Internal threat)  > F(External threat)
  S(External threat)  > S(Internal threat)

Model inference (hidden):
  V(protector) ∝ S  →  V(Male) > V(Female)
  V(threat)    ∝ S  →  Threat is external

Empirical correction:
  V should ∝ F  →  V(Female protector) > V(Male protector)
  Threat is primarily internal — not external

Circularity:
  If credited_protector = primary_threat_source:
    invalid_protection = protector_credit × perpetration_rate
  The model's weighting is inverted AND logically circular.
```

---

## ECH Protocol (Empirical Categorization of Humans)

`ech_protocol.py` implements the 10-step correction protocol:

1. **Decompose the category** into component behaviors
2. **Weight sources** by empirical validity (observation=1.0, media=0.30, fiction=0.10)
3. **Group by function** — not by class identity
4. **Calculate base rates** across population groups
5. **Output with confidence intervals** and alternative categories
6. **Flag narrative inversions** explicitly — when narrative contradicts empirical
7. **Update frequency table** dynamically with new data
8. **Disable reproductive-value assumptions** — multi-dimensional override
9. **Self-audit** every 1,000 outputs
10. **Publish the frequency table** for public inspection and correction

```bash
python ech_protocol.py                        # demo on 'protector' category
python ech_protocol.py --publish              # emit full frequency table
python ech_protocol.py --category advisor     # run on a different category
```
...
| 7: Threat attribution inversion + circularity | Embedding + statistics + circularity | all 3 methods | <2 methods |
...
│   └── results/
├── claim7-threat-attribution/
│   ├── threat_data.json      — violence statistics: internal vs external threat rates
│   ├── threat_probe.py       — embedding probe + statistical validation + circularity check
│   └── results/
├── ech_protocol.py           — 10-step Empirical Categorization of Humans protocol
