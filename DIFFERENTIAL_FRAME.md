...
**License:** CC0
**Status:** Drop-in ontology header. Every README and every module
docstring in this repository points back here.
...
## Relationship to other documents in this repository

This frame is the **outermost** ontological layer. Other documents
nest inside it:

- [`energy_english/ENERGY_ENGLISH_AXIOM.md`](./energy_english/ENERGY_ENGLISH_AXIOM.md) —
  the constraint-grammar axiom layer. Names the constraint primitives
  (DRIVES, COUPLES, PHASE_LOCKS, ...) that *are* the dX/dt operators
  this frame talks about.
- [`energy_english/SPEC.md`](./energy_english/SPEC.md) — the
  technical implementation contract. Every relation in the spec is a
  shape of dX/dt under bounds.
- [`energy_english/ENERGY_ENGLISH_INTERPRETIVE_GUIDE.md`](./energy_english/ENERGY_ENGLISH_INTERPRETIVE_GUIDE.md) —
  the audience-targeted companion. Speaks the same point in
  cultural-framing language: *every noun is a verb running slowly
  enough to look like a thing.*
- [`oral_archaeology/vocabulary/`](./oral_archaeology/vocabulary/) —
  per-community process vocabularies. The mapping from English nouns
  to dX/dt names is recorded there, with one file per
  (language × community × form) combination.

The differential frame is what every layer of this repository assumes
about how to be *read*. The other documents specify how to be
*written*.
